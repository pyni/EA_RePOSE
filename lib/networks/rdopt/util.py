...
def crop_input(inp, bbox):
    ...
    for i in range(bs):
        #print(bbox)
        if bbox[i, 0] < 0 or bbox[i, 1] < 0 or bbox[i, 2]>480 or bbox[i, 3]>640 :
            #print('tttttttttttt1',inp[i, :].shape,bbox[i])
            pad = nn.ZeroPad2d(padding=(paddingnumpy, paddingnumpy, paddingnumpy, paddingnumpy))
            newresult=pad(inp[i, :])
          #  print('tttttttttttt2',newresult.shape )
            inp_r_ = newresult[:, (bbox[i, 0]+paddingnumpy):(bbox[i, 2]+paddingnumpy), (bbox[i, 1]+paddingnumpy):(bbox[i, 3]+paddingnumpy)]
          #  print('tttttttttttt3', inp_r_.shape)
