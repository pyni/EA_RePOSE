...
import numpy as np 
from  lib.networks.rdopt.base_model import BaseModel
from  lib.networks.rdopt.utils import checkpointed
import math
...
class UNet(BaseModel):
    ...
    def build_encoder(self, conf):
        ...
        # elif conf.encoder.startswith('efficientnet'):
        #
    ...
    def _init(self, conf):
        ...
        if conf.compute_uncertainty:
           ...
           # self.scale_branch = nn.Linear(256, 1)

            scale_branch1 = nn.Linear(32, 1)
            scale_branch2 = nn.Linear(64, 1)
            scale_branch = []
            scale_branch.append(scale_branch1)
            scale_branch.append(scale_branch2)
            self.scale_branch = nn.ModuleList(scale_branch)
    ...
    def _forward(self, data): 
        ...
        for adapt, i in zip(self.adaptation, self.conf.output_scales):
            ...
            outputsub=adapt(pre_features[i])
            if self.scales[counti]!=1:
                outputsub=nn.Upsample(scale_factor=self.scales[counti], mode='bilinear',align_corners=False)(outputsub)
            out_features.append(outputsub)
            counti=counti+1
        ...
        if self.conf.compute_uncertainty:
            ...
            for layer, i in zip(self.uncertainty, self.conf.output_scales):
                ...
                if self.scales[counti] != 1:
                    conf = nn.Upsample(scale_factor=self.scales[counti], mode='bilinear',align_corners=False)(conf)
                confidences.append(conf)
                counti = counti + 1
            pred['confidences'] = confidences
        time_end = time.time()
 
        return pred,skip_features 
    def half(self, data): 
        import time
        time_start = time.time()
        image = data 
