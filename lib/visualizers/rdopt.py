...
class Visualizer:
    ...
    def visualize(self, output, batch, step):
        ...
        R = rot_vec_to_mat(output['R']).detach().cpu().numpy()
        t = output['t'].detach().cpu().numpy()[:, :, None]
        ...
        ax.add_patch(
            patches.Polygon(xy=corner_2d_gt[[5, 4, 6, 7, 5, 1, 3, 7]],
                            ...
                            linewidth=1,
                            edgecolor='g'))
        plt.savefig(f'linemod/samples/{cfg.test.dataset}/{cfg.cls_type}/{cfg.cls_type}_{step}.png')
